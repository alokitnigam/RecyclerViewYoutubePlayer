 ...
 */
package com.example.recyclerviewyoutubeplayer.youtubeView.webview;
...
import com.example.recyclerviewyoutubeplayer.BuildConfig;
import com.example.recyclerviewyoutubeplayer.youtubeView.listener.YouTubeEventListener;
import com.example.recyclerviewyoutubeplayer.youtubeView.models.PlayerStateList;
import com.example.recyclerviewyoutubeplayer.youtubeView.util.$Precondition$Check;
...
public class YouTubePlayerWebView extends WebView {
    ...
    private String previousState = PlayerStateList.INSTANCE.NONE;
    ...
    public void resetTime() {
        duration = null;
        currentTime = null;
        previousState = PlayerStateList.INSTANCE.NONE;
    }
    ...
    private void onStateChange(String arg) {
        ...
        if (youTubeListener != null) {
            ...
            if (PlayerStateList.NOT_STARTED.equalsIgnoreCase(arg)) {
                //handle when needed.
            } else if (PlayerStateList.INSTANCE.ENDED.equalsIgnoreCase(arg)) {
                youTubeListener.onStop(getTimeInMilliSec(currentTime), getTimeInMilliSec(duration));
            } else if (PlayerStateList.INSTANCE.PLAYING.equalsIgnoreCase(arg)) {
                if (TextUtils.isEmpty(duration)) {
                    setDuration();
                }
                youTubeListener.onPlay(getTimeInMilliSec(currentTime));
            } else if (PlayerStateList.INSTANCE.PAUSED.equalsIgnoreCase(arg)) {
                youTubeListener.onPause(getTimeInMilliSec(currentTime));
            } else if (PlayerStateList.INSTANCE.BUFFERING.equalsIgnoreCase(arg)) {
                youTubeListener.onBuffering(getTimeInMilliSec(currentTime), true);
            } else if (PlayerStateList.INSTANCE.CUED.equalsIgnoreCase(arg)) {
    ...
    public void stopPlayer() {
        if (isPlayerReady
                && (PlayerStateList.INSTANCE.PLAYING.equals(previousState)
                || PlayerStateList.INSTANCE.BUFFERING.equals(previousState)
                || PlayerStateList.INSTANCE.PAUSED.equals(previousState)
                || PlayerStateList.INSTANCE.CUED.equals(previousState))) {
            stop();
        }
    }
}
